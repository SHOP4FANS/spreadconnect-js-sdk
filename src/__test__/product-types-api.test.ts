import { GetProductTypesResponseMock, GetSingleProductTypeResponseMock, SizeChartResponseMock } from "../__mock__/order-type";
...
describe('Product Types API', () => {
    ...
    test("List product types", async () => {
        ...
        expect(httpClient.request).toHaveBeenCalledWith("GET", `${PRODUCT_TYPES_PATH}`)
    });
    ...
    test("Get product type", async () => {
        ...
        expect(httpClient.request).toHaveBeenCalledWith("GET", `${PRODUCT_TYPES_PATH}/${id}`);
    });
    ...
    test("Get size chart", async () => {
        ...
        expect(httpClient.request).toHaveBeenCalledWith("GET", `${PRODUCT_TYPES_PATH}/${id}/size-chart`);
    })
