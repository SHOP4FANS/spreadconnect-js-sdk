import { CreateSubscriptionPropsMock, CreateSubscriptionResponseMock, DeleteSubscriptionResponseMock, GetSubscriptionsResponseMock, SimulateOrderCancelledResponseMock, SimulateOrderProcessedResponseMock, SimulateShipmentSentResponseMock } from "../__mock__/subscription";
import { SubscriptionsApi } from "../api/subscriptions-api";
import { ORDERS_PATH, SUBSCRIPTIONS_PATH } from "../endpoints/spod-endpoints";
...
describe('Subscriptions API', () => {
    ...
    test("Create subscription", async () => {
        ...
        expect(httpClient.request).toHaveBeenCalledWith("POST", SUBSCRIPTIONS_PATH, CreateSubscriptionPropsMock);
    });
    ...
    test("Delete subscription", async () => {
        ...
        expect(httpClient.request).toHaveBeenCalledWith("DELETE", `${SUBSCRIPTIONS_PATH}/${id}`);
    });

    test("Simulate order cancelled", async () => {
        const id = "10";
        httpClient.request.mockResolvedValueOnce(SimulateOrderCancelledResponseMock);

        const res = await api.simulate_order_cancelled(id);

        expect(res).toEqual(SimulateOrderCancelledResponseMock);
        expect(httpClient.request).toHaveBeenCalledWith("POST", `${ORDERS_PATH}/${id}/simulate/order-cancelled`);
    });

    test("Simulate order processed", async () => {
        const id = "10";
        httpClient.request.mockResolvedValueOnce(SimulateOrderProcessedResponseMock);

        const res = await api.simulate_order_processed(id);

        expect(res).toEqual(SimulateOrderProcessedResponseMock);
        expect(httpClient.request).toHaveBeenCalledWith("POST", `${ORDERS_PATH}/${id}/simulate/order-processed`);
    });
    ...
    test("Simulate shipment sent", async () => {
        ...
        expect(httpClient.request).toHaveBeenCalledWith("POST", `${ORDERS_PATH}/${id}/simulate/shipment-sent`);
    });
